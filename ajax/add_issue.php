...
if($_POST ){
    ...
    $sql = "insert into 'issues' ('latitude','longitude','location_address','no_of_persons','contact_person_name','contact_person_mobile','additional_notes','issue_status','reported_date','updated_date') VALUES('".$latitude."','".$longitude."','".$location."','".$data['noPersons']."','".$data['contactName']."','".$data['contactMobile']."','".$data['notes']."',0,'".$rptTime."','".$rptTime."')";    
    $res= $db->query($sql);
    $db->query("COMMIT");
    if(!$res){
